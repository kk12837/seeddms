...
//				$content .= "<td>".$version."</td>";
			...
			if (file_exists($dms->contentDir . $latestContent->getPath())){
//				$content .= "<a href=\"../op/op.Download.php?documentid=".$docID."&version=".$version."\" title=\"".htmlspecialchars($latestContent->getMimeType())."\"><i class=\"icon-download\"></i> ".getMLText("download")."</a>";
				if ($viewonlinefiletypes && in_array(strtolower($latestContent->getFileType()), $viewonlinefiletypes))
					$content .= "<a target=\"_blank\" href=\"../op/op.ViewOnline.php?documentid=".$docID."&version=".$version."\"><i class=\"icon-star\"></i> " . getMLText("view_online") . "</a>";
			}
